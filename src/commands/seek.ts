...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (position > currentTrack.duration)
      return safeReply(interaction, {
        embeds: [createErrorEmbed(client, '음악의 길이보다 긴 시간으로 건너뛸 수 없어요.', `현재 재생중인 음악의 길이는 ${msToTime(currentTrack.duration)}에요.`)],
      });
    if (player.paused) return safeReply(interaction, {embeds: [createErrorEmbed(client, '일시 정지 상태에서는 건너뛰기를 할 수 없어요.')], flags: MessageFlags.Ephemeral});
    if (currentTrack.isStream) return safeReply(interaction, {embeds: [createErrorEmbed(client, '스트리밍 음악은 건너뛸 수 없어요.')], flags: MessageFlags.Ephemeral});
    if (!currentTrack.isSeekable) return safeReply(interaction, {embeds: [createErrorEmbed(client, '이 트랙은 건너뛰기를 지원하지 않아요.')], flags: MessageFlags.Ephemeral});

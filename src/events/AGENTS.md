...
Six Discord.js event handlers dynamically loaded by `src/utils/core.ts`. Each file exports `event` with `{name, runOnce?, execute} satisfies Event<...>`.
...
| **Boot sequence (once)** | `clientReady.ts` | Registers Lavalink events, restores player state, deploys commands, starts the configured presence interval. Exports `clearPresenceInterval()` called by `NMClient.destroy()`. |
...
- **Type**: `export const event = {...} satisfies Event<'eventName'>` (NOT `as Event`).
...
- **`clientReady` is `runOnce: true`**: it must be idempotent. Re-running it (e.g. after a reconnect) is not supported.

...
/** 중복 인터랙션 처리 방지 Set 자동 정리 타임아웃 (5분) */
const DUPLICATE_CLEANUP_TIMEOUT_MS = 5 * 60 * 1000;
...
export function markInteractionProcessed(interactionId: string): void {
  ...
  /* 5분 후 자동 제거 */
  setTimeout(() => processedInteractions.delete(interactionId), DUPLICATE_CLEANUP_TIMEOUT_MS);
}

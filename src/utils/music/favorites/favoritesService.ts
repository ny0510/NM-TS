...
export async function addFavorite(userId: string, track: QueueTrack): Promise<boolean> {
  ...
  try {
    ...
    logger.error(error instanceof Error ? error : new Error(`즐겨찾기 추가 중 오류가 발생했어요: ${error}`));
    return false;
  }
}
...
export async function removeFavorite(userId: string, trackId: number): Promise<boolean> {
  ...
  try {
    ...
    logger.error(error instanceof Error ? error : new Error(`즐겨찾기 제거 중 오류가 발생했어요: ${error}`));
    return false;
  }
}
...
export async function getUserFavorites(userId: string): Promise<FavoriteTrack[]> {
  ...
  try {
    ...
    logger.error(error instanceof Error ? error : new Error(`즐겨찾기 목록 조회 중 오류가 발생했어요: ${error}`));
    return [];
  }
}
...
export async function isFavorited(userId: string, source: string, identifier: string): Promise<boolean> {
  ...
  try {
    ...
    logger.error(error instanceof Error ? error : new Error(`즐겨찾기 확인 중 오류가 발생했어요: ${error}`));
    return false;
  }
}
...
export async function clearFavorites(userId: string): Promise<number> {
  ...
  try {
    ...
    logger.error(error instanceof Error ? error : new Error(`즐겨찾기 전체 제거 중 오류가 발생했어요: ${error}`));
    return 0;
  }
}
...
export async function getFavoriteCount(userId: string): Promise<number> {
  ...
  try {
    ...
    logger.error(error instanceof Error ? error : new Error(`즐겨찾기 수 확인 중 오류가 발생했어요: ${error}`));
    return 0;
  }
}
